...
Aby to osiągnąć to autoryzacji użytkowników wykorzystano tokeny JWT\@.
...
Każda z tych częsci jest dodatkowo zakodowana algorytmem Base64.
Ostateczna postać tokenu to konkatenacja zakodowanych 3 częsci, częsci są odseparowanie od siebie
...
\begin{itemize}
  ...
  Może to sie stać poprzez wylosowanie liczy, zaszyfrowanie jej kluczem publicznym serwera i
  wysłanie do serwera.
  Następnie na podstawie tej liczny klient i serwer generują klucz symetryczny,
...
W przypadku projektu pracy inżynierskiej danymi poufnymi są hasła do kont użytkowników.
Powinny być one znane tylko dla właścicieli tych kont.
Jeśli tak nie jest, podczas włamania do systemu atakujący może uzyskać hasła i loginy użytkowników
i logować się na ich konta bez ich wiedzy, dodatkowo może również włamać się do innych systemów
jeśli użytkownik użył tam tego samego hasła.
...
Projekt pracy inżynierskiej zakłada jednak, że administrator jest zaufanym użytkownikiem systemu
który nie ma na calu działania na jego szkodę oraz posiada dostęp do serwera
składującego kontenery LXC nie tylko poprzez warstwę widoku projektu.
Użytkownik bez uprawnień administratora nie ma możliwości wprowadzania danych które są następnie
bezpośrednio umieszczanie w lini poleceń, także takie zagrożenie ze strony użytkownika nie istnieje.
...
Aplikacja serwerowa z tego powodu będzie traktowała takie żądania jako prawdziwe.
Poprzez atak CSRF ofiara ataku może nieumyślnie wykonać jaką akcje, np.\ zmodyfikować dane swojego
konta lub wykonać niechciany przelew pieniędzy.
W przypadku projektu pracy inżynierskiej sesja użytkownika nie jest oparta o ciasteczka sesyjne i
...
Istnieje wiele metod zabezpieczanie przed XSS\@.
...
Podstawowe zabezpieczenie przed atakami XSS umożliwia jednak zastosowanie biblioteki React.
...
zabezpieczyć się przed umieszczeniem tam kodu wykonywalnego.
