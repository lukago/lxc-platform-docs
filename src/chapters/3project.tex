...
operacyjnego opartego o jądro Linux~\cite{virtualessential}.
...
maszyny gospodarza.
Może istnieć wiele kontenerów działających jednocześnie w obrębie jednej maszyny
gospodarza~\cite{lxcont}.
...
W systemie Linux kontenery mogą działać dzięki specyficznym funkcjonalnościom jądra
Linux~\cite{lxdpractical}.
...
Dodatkowo zawiera mechanizmy pozwalające zachować integralność danych~\cite{psqlmasterbook}.
...
operacji, zmiany są trwale zapisywane w pamięci~\cite{transactions}.
...
\begin{enumerate}
  ...
  \begin{itemize}
    ...
    \item Serializable.
    Transakcje wykonane współbieżnie będą miały ten sam rezultat, co transakcje
...
z każdą transakcją, która ją modyfikuje~\cite{transactions}.
...
W projekcie pracy inżynierskiej aplikacja serwerowa składa się kilku modułów.
Najważniejsze z modułów wchodzących w skład aplikacji serwerowej to:
...
odzwierciedlenie w aplikacji serwerowej przy zastosowaniu architektury REST\@~\cite{rest}.
...
Kody odpowiedzi HTTP są również ściśle zdefiniowane w specyfikacji protokołu HTTP\@~\cite{rest}.
...
Dzięki takiej oszczędności cały system informatyczny działa wydajniej~\cite{reactpro}.
...
wykorzystana jego zmodyfikowana wersja dla aplikacji internetowych~\cite{springinaction}.
...
wstrzykiwania atrapy obiektów~\cite{springinaction}.
...
Token JWT składa się z trzech części~\cite{jwthandbook}:
...
Ostateczna postać tokenu to konkatenacja zakodowanych trzech części, części są odseparowanie
od siebie znakiem pojedynczej kropki:
...
jawnej dopóki nie trafi do serwera i zostanie przez niego zaszyfrowanie.
...
TLS zapewnia poufność danych poprzez ich odpowiednie szyfrowanie~\cite{http}.
...
w postaci niejawnej~\cite{websecurity}.
...
przekazanym porcie~\cite{websecurity}.
...
konta lub wykonać przelew pieniędzy~\cite{websecurity}.
...
może być wykonany przez przeglądarkę.
Zazwyczaj jest to kod JavaScript, powszechnie używany we wszystkich przeglądarkach.
...
przed tym atakiem, ale znacząco utrudniają jego przeprowadzenie~\cite{websecurity}.
...
w podstawowy sposób zabezpieczyć się przed umieszczeniem tam kodu wykonywalnego~\cite{reactpro}.
