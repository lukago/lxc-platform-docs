...
Następnie skonfigurowano adapter sieciowy dla maszyny wirtualnej, aby umożliwić dostęp
do połączenia przez SSH do maszyny wirtualnej z poziomu lokalnej sieci oraz do kontenerów
LXC znajdujących się na tej maszynie.
...
bazodanowej~\cite{springinaction}.
...
że zostanie wykorzystany domyślny poziom izolacji dla używanego RDBMS\@~\cite{springinaction}.
Dla PostgreSQL jest to poziom \textit{read committed}~\cite{psqlmasterbook},
który jest wystarczający dla tej operacji.

\subsection{ORM}
\label{subsec:orm}

ORM czyli mapowanie relacyjno-obiektowe zostało zrealizowane za pomocą adnotacji JPA\@.
Poniżej przedstawiono przykładową reprezentację tabeli poprzez klasę w języku Java i odpowiednie
adnotacje JPA\@.
...
Znaczenie najważniejszych adnotacji zostało opisane poniżej~\cite{hibernatejpa}:
...
Znaczenie poszczególnych wartości konfiguracji~\cite{lxddocs}:
...
Metody asynchronicznie są oznaczone adnotacją \textit{@Async}~\cite{springinaction}.
...
Zastosowanie interfejsu \textit{IProcessor} umożliwiło brak zależności metod serwisowych
od biblioteki Project Reactor.
...
Do użytkownika wysyłane są powiadomienia o rozpoczętych tylko przez niego akcjach, natomiast
...
Do implementacji połączenia SSH aplikacji serwerowej z serwerem LXC
wykorzystano bibliotekę SSHJ\@.
...
Przedstawiony listing~\ref{lst:ssh} jest metodą, która implementuje interfejs \textit{ICmdRunner},
przedstawiony poniżej we fragmencie~\ref{lst:cmdrunner}.
...
Jeśli zaistniałaby potrzeba połączenia innym protokołem lub uruchamiania
...
Wybieranie implementacji poprzez wartość z konfiguracji zaimplementowano
...
\lstinputlisting[
  caption=Wywołanie metody tworzenia kontenera,
  label=lst:curlcreate,
]{curlcreate.txt}

Na listingu~\ref{lst:curlcreate} wywołano metodę stworzenia kontenera \textit{example123}.
W celu prezentacji działania, wykorzystano narzędzie \textit{cURL}, które wysłało żądanie POST
na odpowiedni adres URI aplikacjo serwerowej.
Widać, że po wywołaniu tego żądania, została zwrócona odpowiedź HTTP 202.

\lstinputlisting[
  caption=Rezultat stworzenia kontenera,
  label=lst:lxcop,
]{lxcop.txt}

Rezultat otrzymanego żądania można zobaczyć na listingu~\ref{lst:lxcop}, pochodzącym z serwera
składującego kontenery LXC\@.
W wyniku żądania tworzona jest operacja tworzenia kontenera, po stworzeniu kontenera
widać go na liście stworzonych kontenerów.

\lstinputlisting[
  caption=Informacje wysyłane przez WebSocket,
  label=lst:websocat,
]{websocat.txt}

Informacje o statusie zadania stworzenia kontenera, są wysyłanie przez gniazda
sieciowe w technologii WebSocket.
Zostało to zaprezentowane na listingu~\ref{lst:websocat} za pomocą programu \textit{websocat},
który łączy się do zarejestrowanego już kanału w aplikacji serwerowej.
...
Taki parametr musi spełnić warunki określone za pomocą adnotacji Bean Validation.
...
żądania HTTP~\cite{springinaction}.

\section{Zabezpieczenia}
\label{sec:zabezpieczenia}

Aplikacja została zabezpieczona na kilka sposobów.
Jest to między innymi mechanizm uwierzytelniania za pomocą JWT oraz wymaganie protokołu HTTPS\@.
Do implementacji tych mechanizmów wykorzystano wiele technologii np.\ szkielet Spring czy
bibliotekę JJWT\@.

Przy każdym żądaniu wysłanym na zabezpieczony punkt dostępowy, aplikacja pobiera nagłówki żądania.
Do autoryzacji wymagany jest nagłówek \textit{Authorization} zawierający token JWT\@,
który jest następnie walidowany.
...
Do weryfikacji nagłówka z tokenem JWT wykorzystano filtr, będący częścią
specyfikacji \textit{Java Servlet}.
Filtruje on każde żądanie jeszcze przed obsłużeniem go przez kontrolery.
Implementacja filtra została przedstawiona na listingu~\ref{lst:jwtfilter}.

Dodatkowo skonfigurowano szkielet Spring w taki sposób, aby wymagał zabezpieczeń tylko na
określonych adresach oraz do autoryzacji używał wcześniej opisanego filtra.

\lstinputlisting[
  caption=Konfiguracja zabezpieczeń szkieltu Spring,
  label=lst:springsec,
  language=Java,
]{springsec.txt}

Na listingu~\ref{lst:springsec} została przedstawiona konfiguracja zabezpieczeń szkieletu Spring.
Skonfigurowana została bezstanowość sesji za pomocą \textit{SessionPolicy}, wykorzystanie filtra JWT
za pomocą \textit{JwtTokenFilterConfigurer} oraz adresy wymagające autoryzacji za pomocą metody
\textit{authenticated}.
Za pomocą metody \textit{requiresSecure} wymuszono wykorzystanie protokołu HTTPS
na wszystkich metodach dostępowych aplikacji serwerowej.

Dodatkowo zabezpieczono również rejestrację kanału WebSocket.
Tam również jest wykorzystywany nagłówek HTTP z tokenem JWT, który następie jest weryfikowany.
Obsługa tego zabezpieczenia również jest oparta o szkielet Spring, poprzez przeciążenie odpowiednich
metod konfiguracyjnych.
Implementacja tego zabezpieczenia została przedstawiona na listingu~\ref{lst:socketjwt}

\lstinputlisting[
  caption=Zabezpieczenia dla WebSocket,
  label=lst:socketjwt,
  language=Java,
]{socketjwt.txt}

\section{Warstwa widoku}
\label{sec:warstwa-widoku}

Widok użytkownika, wykonany w technologii React, jest reaktywny
i odświeża tylko wybrane elementów na stronie, zgodnie z techniką
SPA\@.
W tej sekcji zostaną zaprezentowane poszczególne widoki wraz z
ich opisem.

\subsection{Panel administratora}
\label{subsec:panel-administratora}

Projekt systemu zakłada odrębne poziomy dostępu dla administratora i klienta.
Z tego powodu warstwa widoku dla administratora wygląda inaczej od warstwy widoku
klienta oraz posiada inne funkcje.

\begin{figure}[H]
  \centering
  \fcolorbox{frame}{frame}{\includegraphics[scale=0.32]{figures/lxclist2.png}}
  \caption{Lista kontenerów}
  \label{fig:lxclist}
\end{figure}

Zrzut z ekranu~\ref{fig:lxclist} przedstawia listę wszystkich kontenerów.
Jest to pierwsza strona widoczna po zalogowaniu się na konto administratora,
dostępna również z poziomu menu bocznego \textit{Panel Główny}.
Z poziomu tego widoku można dodawać nowe kontenery.
Menu boczne dostarcza również opcję podglądu wszystkich zadań zrealizowanych na
istniejących w systemie kontenerach (przycisk \textit{Zadania}) oraz wyświetlenia listy
wszystkich użytkowników i ich edycję (przycisk \textit{Użytkownicy}).

\begin{figure}[H]
  \centering
  \fcolorbox{frame}{frame}{\includegraphics[scale=0.32]{figures/lxcdetails2.png}}
  \caption{Strona logowania}
  \label{fig:lxcdetails}
\end{figure}

Zrzut z ekranu~\ref{fig:lxcdetails} przedstawia szczegóły pojedynczego kontenera.
W szczegóły można wejść poprzez link w wierszu listy kontenerów~\ref{fig:lxclist}.
Można tutaj zarządzać stanem kontenera i przypisać do niego użytkownika.

\subsection{Panel klienta}
\label{subsec:panel-klienta}

Widok dla kont z poziomem dostępu klienta jest podobny do panelu administratora,
jednak posiada ograniczenia, zgodne z opisem przypadków użycia w tabeli~\ref{tab:przypadki_uzycia}.
Poniżej zaprezentowano najważniejsze elementy widoku.

\begin{figure}[H]
  \centering
  \fcolorbox{frame}{frame}{\includegraphics[scale=0.32]{figures/userlxclist2.png}}
  \caption{Strona główna z listą przypisanych kontenerów}
  \label{fig:userhome}
\end{figure}

Zrzut z ekranu~\ref{fig:userhome} przedstawia listę przypisanych kontenerów do konta.
Jest to pierwsza strona widoczna po zalogowaniu się na konto klienta,
dostępna również z poziomu menu bocznego \textit{Panel Główny}.
Menu dostarcza również opcję podglądu wszystkich zadań zrealizowanych na przypisanych do swojego
konta kontenerach (przycisk \textit{Zadania}) oraz edycji swojego profilu, w tym zmiany
hasła (przycisk \textit{Profil}).

Po włączeniu kontenera użytkownik może zalogować się do kontenera poprzez SSH,
za pomocą zaprezentowanego adresu i portu.
Poprawne zalogowanie się na przykładowe konto \textit{user} przedstawiono na
poniższym listingu.
Do zalogowania się użytkownik wykorzystuje dane kontenera zaprezentowanie na
rys.~\ref{fig:userhome}.

\lstinputlisting[
  caption=Logowanie do kontenera przez protokół SSH,
  label=lst:sshinto,
]{sshinto.txt}

\subsection{Internacjonalizacja}
\label{subsec:internacjonalizacja}

Warstwa widoku jest dostępna w języku polskim oraz w języku angielskim.
Przykład działania internacjonalizacji przedstawiono na poniższych
zrzutach z ekranu ze strony logowania.

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.45]{figures/i18npl.png}
  \caption{Strona logowania w języku polskim}
  \label{fig:i18npl}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[scale=0.45]{figures/i18en.png}
  \caption{Strona logowania w języku angielskim}
  \label{fig:i18en}
\end{figure}

Mechanizm internacjonalizacji osiągnięto poprzez trzymanie wszystkich wartości tekstowych
napisanych w danym języku w osobnym pliku.
Następnie wybierana jest zawartość pliku z wartościami w języku polskim albo angielskim,
na podstawie wartości \textit{NavigatorLanguage.language} obecnej we wszystkich
aktualnych przeglądarkach internetowych.
